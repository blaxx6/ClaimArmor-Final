...
class AuthenticatedApiTests(unittest.TestCase):
    ...
    def test_csv_to_review_queue_to_writeback(self):
        ...
        uploaded = self.client.post(
            "/api/claims/upload-csv", headers=analyst, json={"csv_text": csv_text}
        )
        self.assertEqual(uploaded.status_code, 200, uploaded.text)
        self.assertEqual(uploaded.json()["summary"]["created"], 1)
        investigated = self.client.post(
            "/api/claims/CLM-API-CSV/investigate", headers=analyst
        )
        ...
        reviewed = self.client.post(
            "/api/investigations/CLM-API-CSV/review",
            headers=reviewer,
            json={
                "reviewer": "ignored",
                "action": "APPROVE",
                "reason": "Verified cited accident evidence",
                "final_route": "HOLD",
            },
        )
    ...
    def test_streamed_investigation_emits_real_agent_order_and_result(self):
        ...
        with self.client.stream(
            "POST", "/api/claims/CLM-HOLD-001/investigate-stream", headers=analyst
        ) as response:
            self.assertEqual(response.status_code, 200)
            events = [json.loads(line) for line in response.iter_lines() if line]
        agents = [
            event["data"]["agent"] for event in events if event["type"] == "agent"
        ]
        self.assertEqual(
            agents,
            [
        ...
        self.assertEqual(
            events[-1]["data"]["recommended_primary_payer"], "AUTO_INSURER"
        )
