...
db.put_investigation("test_claim_logic_2", result)
...
with engine.connect() as conn:
    ...
    query = select(db.investigations_table.c.result).outerjoin(
        ...
        db.investigations_table.c.claim_id == 'test_claim_logic_2',
    ...
    rows0 = conn.execute(query).all()
    print(f"Pending BEFORE review: {len(rows0)}")

# A review is made
db.put_review("test_claim_logic_2", {"action": "HOLD"})
