...
def _security_checks() -> dict:
    ...
    return {
        "cases": len(attacks),
        "blocked": blocked,
        "prompt_injection_block_rate": round(blocked / len(attacks), 4),
        "markers_tested": list(INJECTION_MARKERS),
    }
...
def evaluate_full(output: Path = OUTPUT) -> dict:
    ...
    for claim in DEMO_CLAIMS:
        ...
        evidence_supported = (
            bool(result["evidence"]) if result["route"] in {"HOLD", "CLEAR"} else True
        )
        cases.append(
            {
    ...
    route_accuracy = sum(
        item["expected_route"] == item["actual_route"] for item in cases
    ) / len(cases)
    payer_cases = [item for item in cases if item["expected_payer"]]
    payer_accuracy = sum(
        item["expected_payer"] == item["actual_payer"] for item in payer_cases
    ) / len(payer_cases)
    ...
    result = {
        ...
        "end_to_end": {
            ...
            "citation_correctness_proxy": round(
                sum(item["evidence_supported"] for item in cases) / len(cases), 4
            ),
            "unsupported_answer_rate": round(
                sum(not item["evidence_supported"] for item in cases) / len(cases), 4
            ),
            "human_agent_agreement_proxy": round(route_accuracy, 4),
            "average_processing_ms": round(statistics.mean(latencies), 2),
            "p95_processing_ms": round(
