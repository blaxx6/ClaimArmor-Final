...
def _score(name: str, labels, predictions, probabilities, amounts) -> dict:
    ...
    return {
        ...
        "value_weighted_recall": round(prevented / total_leakage, 4)
        if total_leakage
        else 0,
        "simulated_review_cost": round(review_cost, 2),
        "simulated_false_positive_cost": round(false_positive_cost, 2),
        "simulated_net_benefit": round(
            prevented - review_cost - false_positive_cost - operating_cost, 2
        ),
    }
...
def evaluate(
    ...
    definite_rule = ((frame["accident_related"] == 1) & (frame["has_auto"] == 1)) | (
        (frame["has_medicare"] == 1)
        & (frame["has_employer"] == 0)
        & (frame["submitted_is_employer"] == 1)
    )
    overlap_rule = (
        (frame["coverage_overlap"] == 1)
        & (frame["has_medicare"] == 1)
        & (frame["has_employer"] == 1)
    )
    ...
    hybrid_predictions = (
        (
            definite_rule
            | (overlap_rule & (probabilities >= 0.35))
            | (probabilities >= 0.72)
        )
        .astype(int)
        .to_numpy()
    ...
    hybrid_probability[overlap_rule.to_numpy()] = hybrid_probability[
        overlap_rule.to_numpy()
    ].clip(min=0.65)
    ...
    result = {
        ...
        "assumptions": {
            "review_cost_per_flag": 35,
            "false_positive_delay_cost": 75,
            "processing_cost_per_claim": 0.20,
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare ClaimArmor decision approaches"
    )
