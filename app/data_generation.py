...
def generate_entity_bundle(member_count: int = 250, seed: int = 42) -> dict:
    ...
    employers = [
        {
            "employer_id": f"EMP-{i:03d}",
            "name": f"Synthetic Employer {i}",
            "employee_count": rng.choice([8, 16, 24, 75, 120, 800]),
        }
        for i in range(1, 31)
    ]
    providers = [
        {
            ...
            "name": f"Synthetic {'Hospital' if i % 3 == 0 else 'Clinic'} {i}",
            "kind": "HOSPITAL" if i % 3 == 0 else "CLINIC",
        }
        for i in range(1, 26)
    ]
    ...
    for index in range(1, member_count + 1):
        ...
        member = {
            ...
            "name": name,
            "dob": dob.isoformat(),
            "address": f"{rng.randint(1, 300)} {rng.choice(STREETS)}",
            "email": f"{first}.{last}.{index}@example.test".lower(),
            "phone": f"+1-555-{index:04d}",
            "employer_id": employer["employer_id"],
            "employment_active": rng.random() > 0.12,
            ...
            dependants.append(
                {
                    ...
                    "name": f"{rng.choice(FIRST)} {last}",
                    "relationship": rng.choice(["SPOUSE", "CHILD"]),
                    "dob": (dob + timedelta(days=rng.randint(6500, 13000))).isoformat(),
                }
            )
        employer_coverage = {
            "coverage_id": f"COV-E-{index:06d}",
            "member_id": member_id,
            "payer": "EMPLOYER_PLAN",
            "kind": "EMPLOYER",
            "start": "2025-01-01",
        ...
        if accident:
            ...
            medicare and member["employment_active"] and employer["employee_count"] < 20
        ):
            primary = "MEDICARE"
        else:
            primary = "EMPLOYER_PLAN"
        claim = {
            "claim_id": f"ENT-CLM-{index:06d}",
            "member_name": name,
            ...
            "amount": round(rng.uniform(250, 75000), 2),
            "submitted_payer": "EMPLOYER_PLAN",
            "claim_type": "TRAUMA" if accident else "MEDICAL",
            "accident_related": accident,
            "diagnosis_group": "ACCIDENT" if accident else "GENERAL",
            "provider_id": rng.choice(providers)["provider_id"],
            "employer_id": employer["employer_id"],
            "employment_active": member["employment_active"],
            "employer_size": employer["employee_count"],
            "disability": member["disability"],
            "relationship": "SELF",
            "expected_primary_payer": primary,
            "overpayment_label": int(primary != "EMPLOYER_PLAN"),
        }
        claims.append(claim)
    ...
    return {
        "metadata": {"synthetic": True, "seed": seed, "member_count": member_count},
        "employers": employers,
        "providers": providers,
        "members": members,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("artifacts/synthetic_entities.json")
    )
