...
def _trace(state: InvestigationState, agent: str, output: Any) -> list[dict]:
    return [
        *state.get("trace", []),
        {"agent": agent, "status": "complete", "output": output},
    ]
...
def coverage_node(state: InvestigationState) -> dict:
    ...
    summary = (
        ", ".join(f"{item['payer']} ({item['kind']})" for item in active)
        or "no active coverage"
    )
    return {
        "trace": _trace(state, "coverage_investigator", f"Active coverage: {summary}.")
    }
...
def research_node(state: InvestigationState) -> dict:
    ...
    fallback = {
        "applicable_policy_ids": [item["policy_id"] for item in evidence],
        "findings": ["Offline retrieval completed; provider analysis unavailable."],
        "missing_facts": [],
        "confidence": 0.0,
    }
    analysis, provider = run_structured_agent(
        ...
        {
            "claim": state["claim"],
            "coverage_timeline": state["timeline"],
            "retrieved_evidence": [
                {
    ...
    return {
        ...
        "trace": _trace(state, "policy_researcher", output),
    }
...
def reasoning_node(state: InvestigationState) -> dict:
    ...
    review = next(
        (rule for rule in state["rules"] if rule["outcome"] == "REVIEW"), None
    )
    ...
    proposal, provider = run_structured_agent(
        ...
        {
            "claim": state["claim"],
            "member_match": state["match"],
            "coverage_timeline": state["timeline"],
            "deterministic_rules": state["rules"],
            "policy_analysis": state.get("policy_analysis", {}),
            "available_policy_ids": [
                item["policy_id"] for item in state.get("evidence", [])
            ],
        },
        fallback,
        provider="groq",
    ...
    output = {
        "deterministic_gate": {
            "route": route.value,
            "recommended_primary_payer": payer,
    ...
    return {
        ...
        "payer": payer,
        "confidence": confidence,
        "ai_primacy": {"proposal": proposal, "provider": provider},
        "trace": _trace(state, "primacy_reasoner", output),
    }
...
def verify_node(state: InvestigationState) -> dict:
    ...
    if (
        state["claim"].get("accident_related")
        and state["route"] == DecisionRoute.HOLD
        and "CMS-MSP-LIABILITY-001" not in evidence_ids
    ):
        contradictions.append(
            "The accident decision lacks the expected liability/no-fault evidence."
        )
    if state["match"]["confidence"] < 0.85:
        contradictions.append(
            "Member identity confidence is below the automated-decision threshold."
        )
    fallback = {
        "citation_supported": bool(evidence_ids),
        "validated_policy_ids": [],
        "contradictions": [],
        "missing_facts": [],
    ...
    critique, provider = run_structured_agent(
        ...
        "Challenge the proposed decision. Return citation_supported (boolean), validated_policy_ids, contradictions, missing_facts, requires_human_review (boolean), and confidence. A disagreement must reference supplied facts or policy IDs.",
        {
            "claim": state["claim"],
            "coverage_timeline": state["timeline"],
            "deterministic_route": state["route"].value,
            "deterministic_payer": state.get("payer"),
            "ai_primacy": state.get("ai_primacy", {}),
            "evidence": [
                {"policy_id": item["policy_id"], "text": item["text"]}
    ...
    if provider.get("used"):
        proposed_payer = (
            state.get("ai_primacy", {}).get("proposal", {}).get("primary_payer")
        )
        if (
            proposed_payer
            and state.get("payer")
            and proposed_payer != state.get("payer")
        ):
            contradictions.append(
                f"AI primacy proposal ({proposed_payer}) conflicts with the deterministic payer ({state.get('payer')})."
            )
        invalid_ids = set(critique.get("validated_policy_ids", [])) - evidence_ids
        if invalid_ids:
            contradictions.append(
                "Verifier cited policy IDs that were not retrieved; human review is required."
            )
        contradictions.extend(
            f"AI critic: {item}"
            for item in critique.get("contradictions", [])[:5]
            if isinstance(item, str)
        )
        if critique.get("requires_human_review") and not critique.get("contradictions"):
            contradictions.append(
                "AI critic requested human review because evidence or claim facts remain incomplete."
            )
    route = DecisionRoute.HUMAN_REVIEW if contradictions else state["route"]
    confidence = (
        min(state["confidence"], 0.60) if contradictions else state["confidence"]
    )
    output = {
        "passed": not contradictions,
        "contradictions": contradictions,
        "checked_policy_ids": sorted(evidence_ids),
    ...
    return {
        ...
        "confidence": confidence,
        "contradictions": contradictions,
        "ai_critique": {"critique": critique, "provider": provider},
        "trace": _trace(state, "verification_critic", output),
    }
...
def financial_node(state: InvestigationState) -> dict:
    ...
    financial = {
        ...
        "estimated_review_cost": 35.0,
        "expected_net_value": round(max(amount_at_risk - 35, 0), 2),
    }
    return {
        "financial": financial,
        "trace": _trace(state, "financial_impact", financial),
    }
...
def explanation_node(state: InvestigationState) -> dict:
    ...
    context = {
        ...
        "policy_analysis": state.get("policy_analysis", {}),
        "ai_primacy": state.get("ai_primacy", {}),
        "ai_critique": state.get("ai_critique", {}),
        "evidence": [
            {"policy_id": item["policy_id"], "text": item["text"]}
            for item in state.get("evidence", [])
        ],
    ...
    return {
        ...
        "llm": llm,
        "trace": _trace(state, "explanation", {"text": explanation, "provider": llm}),
    }
...
def run_agents(
    claim: dict, match: dict, timeline: list[dict], risk: dict, rules: list[dict]
) -> dict:
    state = GRAPH.invoke(
        {
            "claim": claim,
